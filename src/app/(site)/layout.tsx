...
// Safe locale resolution: try next-intl's getLocale, fallback to URL extraction
async function getSafeLocale(): Promise<Locale> {
  // 1. Try extracting from headers first (most reliable)
  try {
    const requestHeaders = await headers();

    // Try x-next-intl-locale header (set by middleware)
    ...
    // Headers access can fail in some contexts
  }

  // 2. Try next-intl's getLocale as fallback
  try {
    ...
    // getLocale() can throw if context is not available
  }
...
export default async function SiteLayout({ children }: { children: React.ReactNode }) {
  ...
  const siteUrl =
    process.env.NEXT_PUBLIC_SITE_URL || "https://abdelbadie-khoubiza.com";
